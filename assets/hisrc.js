...
(function($) {
  ...
  $.fn.hisrc = function(options) {
    ...
    var speedTestUri = settings.speedTestUri,
      ...
      // may as well piggy-back on Forsight localstorage key since we're doing the same thing.
      ...
      initSpeedTest = function() {
        ...
        speedTestImg = document.createElement('img'),
          endTime, startTime, speedTestTimeoutMS;
      ...
      setImageSource = function($el, src) {
        if (settings.useTransparentGif) {
          $el.attr('src', settings.transparentGifSrc).css('max-height', '100%').css('max-width', '100%').css('background', 'url("' + src + '") no-repeat 0 0').css('background-size', 'contain');
        } else {
          $el.attr('src', src);
        }
      };
